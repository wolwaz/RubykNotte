...
- [ ] Open Recovery Backup → prompts to load, opens content in new Untitled tab (marked modified)
...
- [ ] Type `*` elsewhere (not on tracked closer) → inserts `**` pair, cursor between
...
- [ ] Max 10 tracked marks → typing 11+ pairs unsets older marks (memory protection)
...
- [ ] Move line with single-line selection → selection moves with it
- [ ] Move line with multi-line selection → entire block moves, multi-line selection preserved
...
| Auto-close logic      | All 4 pair types, skip behavior, reset on open, mark limits |
| `move_line_up/down`   | Single & multi-line selection preservation, cursor column, first/last line |
| `save_file` / `save_as_file` | New file, existing file, overwrite, read-only disk  |
| `apply_theme`         | Both themes, all widgets, re-parse                   |
| `replace_all`         | Cursor preservation, scroll preservation, backrefs   |
| Backup logic          | Idle time triggers, crash recovery, open recovery    |
