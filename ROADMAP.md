...
- [x] Line-based Markdown highlight: H1–H6, bold, italic, bold+italic, inline code, strikethrough, blockquotes, horizontal rules
...
- [x] Zoom, adjustable line spacing, and editor padding
- [x] Go to Line
...
- [x] Auto-pairing for `*`, `` ` ``, `[]`, `()` with mark tracking (`*` / `` ` `` always scaffold)
...
- [x] Horizontal-rule Enter shortcut from a scaffolded `***` pair

#### Safety (baseline)
- [x] Emergency crash recovery under `~/.markdown_editor_backups/`
- [x] Atomic backup write; primary + `.bak` rotation (including on startup)
- [x] Open Recovery Backup menu action (does not rotate away the chosen file)
- [x] Crash handler attempts emergency save before exit
- [x] Unsaved-changes prompt on Quit, New, Open, and Recovery
...
### Still rough in v0.3.0 (known gaps)
...
- [x] Unsaved-change confirmation for New and Open
...
- [x] Triple-asterisk bold+italic highlighting (nested mixed spans still limited)
...
- [x] H1–H6, bold, italic, bold+italic
- [x] Basic list continuation and auto-pairing
- [x] Inline code, blockquotes, horizontal rules
- [ ] Highlight links, images, fenced code, lists, task lists, tables
...
Crash recovery in v0.3.0 is a floor, not the ceiling.

- [x] Emergency / idle backup to recovery files
- [x] Crash-time emergency save attempt
- [x] Unsaved-change confirmation on New / Open / Recovery / Quit
